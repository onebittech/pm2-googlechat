...
const urlRouter = {
  /**
   * Keys are urls, values are instances of MessageQueue
   *
  ...
  /**
   * Add the message to appropriate message queue (each URL has own independent message enqueing).
   *
   * @param {Message} message
   */
  addMessage: function (message) {
    const processName = message.name;
    const url = moduleConfig["url-" + processName] || moduleConfig["url"];

    if (!url) {
      return;
      // No URL defined for this process and no global URL exists.
    }

    if (!this.messageQueues[url]) {
      // Init new messageQueue to different URL.
      ...
      configProperties.map((configPropertyName) => {
        ...
        config[configPropertyName] =
          moduleConfig[configPropertyName + "-" + processName] ||
          moduleConfig[configPropertyName];
      });

      this.messageQueues[url] = new MessageQueue(config);
    }

    this.messageQueues[url].addMessageToQueue(message);
  },
};
...
function parseProcessName(process) {
  return (
    process.name +
    (process.exec_mode === "cluster_mode" && process.instances > 1
      ? `[${process.pm_id}]`
      : "")
  );
}
